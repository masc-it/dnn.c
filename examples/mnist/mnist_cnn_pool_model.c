...
mnist_model_cnn_pool *mnist_model_create_cnn_pool(struct mem_pool *params) {
    mnist_model_cnn_pool *m = _mem_pool_alloc(params, sizeof(mnist_model_cnn_pool), NULL);
    module_init(&m->base, params, "mnist_model_cnn_pool");

    m->conv1 = conv2d_create(params, 1, 32, 3, 1, 1);
    module_add_child(&m->base, "conv1", &m->conv1->base);

    m->conv2 = conv2d_create(params, 32, 64, 3, 1, 1);
    module_add_child(&m->base, "conv2", &m->conv2->base);

    m->conv3 = conv2d_create(params, 64, 64, 3, 1, 1);
...
void mnist_train_cnn_pool(struct dnn_ctx *ctx, mnist_model_cnn_pool *m,
    ...
    mnist_train_impl(ctx, train_images, train_labels,
                     ...
                     &m->base, mnist_model_forward_cnn_pool);
}
