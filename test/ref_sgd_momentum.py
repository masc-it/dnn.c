...
# dependencies = ["torch"]
# ///
"""PyTorch reference: SGD with momentum on XOR MLP.

Uses IDENTICAL initial weights as C's `tensor_uniform(seed=42)`
so loss values must match exactly step-for-step.

Usage:
  uv run --script test/ref_sgd_momentum.py
"""
import math
import torch
...
with torch.no_grad():
    ...
    w2 = model[2].weight
    b2 = model[2].bias
    # C stores weight (in, out), PyTorch stores (out, in) → transpose
    w1.copy_(torch.tensor(l1_w).reshape(2, 16).T)
    b1.copy_(torch.tensor(l1_b))
    w2.copy_(torch.tensor(l2_w).reshape(16, 2).T)
    b2.copy_(torch.tensor(l2_b))

lr = 0.5
momentum = 0.9
opt = torch.optim.SGD(model.parameters(), lr=lr, momentum=momentum)

print("── XOR MLP (PyTorch, identical init as C) ──")
...
with torch.no_grad():
    ...
    for i in range(4):
        ...
        print(f"    [{int(X[i,0])} {int(X[i,1])}] → {int(preds[i])}  (true {int(y[i])}) {match}")
    print("  done.")
