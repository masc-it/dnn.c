...
int main(void) {
    ...
    dnn_ctx_init(&ctx, 12 * 1024 * 1024, 128 * 1024 * 1024, 1 * 1024 * 1024);
    ...
    tensor *y = tensor_zeros_data(ctx.data, 1, (int[]){N});
    ...
    for (int w = 0; w < 3; w++) {
        ...
        tensor *h = mnist_model_forward_cnn(ctx.scratch, m, x);
        tensor *l = tensor_cross_entropy(ctx.scratch, h, y, 1);
    ...
    m = mnist_model_create_cnn(ctx.params);
    ...
    h = linear_forward(ctx.scratch, m->fc1, h);
    t1 = now_ms(); printf("  fc1:     %6.2f ms\n", t1 - t0); t0 = t1;

    h = tensor_relu(ctx.scratch, h);
    h = tensor_dropout(ctx.scratch, h, 0.5f);
    h = linear_forward(ctx.scratch, m->fc2, h);
    ...
    x = tensor_scratch(ctx.scratch, 2, (int[]){N, 784}, 0);
    for (int i = 0; i < N*784; i++) ((float*)x->data)[i] = ((float)rand()/RAND_MAX)*2-1;
    ...
    tensor *loss = tensor_cross_entropy(ctx.scratch, h, y, 1);
