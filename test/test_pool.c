...
/* _mem_pool_alloc is internal but needed here to test raw pool alloc */
...
static void test_pool_alloc_copies_source_data(void) {
    ...
    float *dst = _mem_pool_alloc(&pool, 3 * sizeof(float), src);
...
static void test_pool_reset_zeroes_next_allocation(void) {
    ...
    float *first = _mem_pool_alloc(&pool, 4 * sizeof(float), NULL);
    ...
    float *second = _mem_pool_alloc(&pool, 4 * sizeof(float), NULL);
